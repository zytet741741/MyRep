	...
	<table class="table table-bordered table-striped" id="mytable" border="1">
		<tr>
	 	...
	 	<%-- <c:if test="${allSale !=null }">   --%>
			<c:forEach items="${Evaluated }" var="Evaluated">
				<tr>
					<td>${Evaluated.good_name }</td>
					<td>${Evaluated.price }</td>
					<td>${Evaluated.order.count }</td>
					<td>${Evaluated.orderType.status_name }</td>
					...
					<td>
						<div class="btn-group">
							...
							<ul class="dropdown-menu">
								<li><a href="##">详情</a></li>
								 <c:if test="${Evaluated.order.status == 1}"> 
									<li><a href="${Evaluated.request.contextPath}/goods/ToPay.do?orderId=${allSale.order.id}">立即付款</a></li>
								 </c:if>
								<c:if test="${Evaluated.order.status == 3}">
									<li><a href="${pageContext.request.contextPath}/goods/ToReceived.do?orderId=${allSale.order.id}">确认收货</a></li>
								</c:if>
								<c:if test="${Evaluated.order.status == 4}">
									<li><a href="##">立即评价</a></li>
								</c:if>
							</ul>
						</div>
