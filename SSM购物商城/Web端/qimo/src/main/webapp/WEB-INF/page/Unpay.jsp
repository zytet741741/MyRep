	...
	<table class="table table-bordered table-striped" id="mytable" border="1">
		<tr>
	 	...
	 	<%-- <c:if test="${allSale !=null }">   --%>
			<c:forEach items="${Unpay }" var="Unpay">
				<tr>
					<td>${Unpay.good_name }</td>
					<td>${Unpay.price }</td>
					<td>${Unpay.order.count }</td>
					<td>${Unpay.orderType.status_name }</td>
